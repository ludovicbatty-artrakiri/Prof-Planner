...
function uid() {
  // Génère un vrai UUID v4, requis par les colonnes "uuid" de Supabase
  // (l'ancien format court provoquait un rejet silencieux "400 Bad Request" sur chaque sauvegarde).
...
// Filet de sécurité supplémentaire (utile surtout sur navigateur desktop ;
// peu fiable sur PWA mobile fermée d'un coup, d'où le await save(db) partout ailleurs).
...
let db = { timeSlots: [], classes: [], students: [], courses: [], preparations: [], tasks: [], evaluations: [], events: [], links: [], documents: [], rooms: [], seatingPlans: [], seatingConstraints: [] };
...
function pageHead(main, eyebrow, title, actionHtml) {
  ...
  head.innerHTML = `
    ...
    <div style="display:flex;align-items:flex-end;gap:16px;">
      ${actionHtml || ""}
      <button class="btn btn-ghost btn-sm" id="force-sync-btn" title="Forcer la sauvegarde en ligne maintenant">💾 Enregistrer & synchroniser</button>
  ...
  const syncBtn = head.querySelector("#force-sync-btn");
  syncBtn.addEventListener("click", async () => {
    if (!currentUser) {
      showToast("Connecte-toi pour synchroniser en ligne.");
      return;
    }
    ...
    await save(db);
    syncBtn.textContent = "✓ Synchronisé, tu peux fermer";
    setTimeout(() => {
...
/* ================= SALLES (plans de tables, vue du dessus) ================= */
const DESK_W = 62, DESK_H = 40;
const CANVAS_MIN_W = 320, CANVAS_MIN_H = 220;
const CANVAS_DEFAULT_W = 720, CANVAS_DEFAULT_H = 420;
const CANVAS_MAX_W = 2400, CANVAS_MAX_H = 1600;
const BOARD_W = 130, BOARD_H = 18;
const TEACHER_DESK_W = 76, TEACHER_DESK_H = 44;
const ADJACENT_DIST = 92; // distance (px) en dessous de laquelle deux tables sont considérées "l'une à côté de l'autre"

let roomsEditingId = null;
let planZoom = Number(localStorage.getItem("profplanner_plan_zoom")) || 1;

function toggleZoomButtonHtml() {
  return `<button class="btn btn-ghost btn-sm" id="zoom-toggle-btn">${planZoom > 1 ? "🔍 Taille normale" : "🔍 Agrandir le plan"}</button>`;
}
function bindZoomToggle(main) {
  main.querySelector("#zoom-toggle-btn").addEventListener("click", () => {
    planZoom = planZoom > 1 ? 1 : 1.6;
    localStorage.setItem("profplanner_plan_zoom", String(planZoom));
    render();
  });
}
// Enveloppe le canevas dans un conteneur dont la taille réelle suit le zoom,
// pour que le défilement (overflow) fonctionne correctement une fois agrandi.
function wrapCanvasForZoom(canvasWrap, canvas, canvasW, canvasH) {
  canvas.style.transform = `scale(${planZoom})`;
  canvas.style.transformOrigin = "top left";
  const sizer = document.createElement("div");
  sizer.className = "room-canvas-sizer";
  sizer.style.width = (canvasW * planZoom) + "px";
  sizer.style.height = (canvasH * planZoom) + "px";
  sizer.style.position = "relative";
  sizer.appendChild(canvas);
  canvasWrap.appendChild(sizer);
  return sizer;
}
...
function openRoomNameModal() {
  openModal(`
    ...
    <div class="field"><label>Nom de la salle</label><input id="f-room-name" placeholder="Salle 204"></div>
    <div class="modal-actions">
      <button class="btn btn-ghost" id="f-cancel">Annuler</button>
  ...
  document.getElementById("f-save").onclick = async () => {
    ...
    const room = {
      ...
      board: { x: (CANVAS_DEFAULT_W - BOARD_W) / 2, y: 14 },
      teacherDesk: { x: (CANVAS_DEFAULT_W - TEACHER_DESK_W) / 2, y: 44 },
    };
    db.rooms.push(room);
    await save(db);
    closeModal();
    roomsEditingId = room.id;
    render();
  };
}

function ensureRoomFurniture(room) {
  if (!room.canvasW) room.canvasW = CANVAS_DEFAULT_W;
  if (!room.canvasH) room.canvasH = CANVAS_DEFAULT_H;
  if (!room.board) room.board = { x: (room.canvasW - BOARD_W) / 2, y: 14 };
  if (!room.teacherDesk) room.teacherDesk = { x: (room.canvasW - TEACHER_DESK_W) / 2, y: 44 };
  return room;
}

function renderRooms(main) {
  if (roomsEditingId && db.rooms.some((r) => r.id === roomsEditingId)) {
    renderRoomEditor(main, db.rooms.find((r) => r.id === roomsEditingId));
    return;
  }

  pageHead(main, "Organisation", "Salles",
    `<button class="btn btn-primary" id="add-room-btn">+ Nouvelle salle</button>`);
  main.querySelector("#add-room-btn").addEventListener("click", () => openRoomNameModal());

  if (db.rooms.length === 0) {
    main.appendChild(emptyState("Aucune salle", "Crée une salle pour dessiner son plan de tables, vu du dessus."));
    return;
  }
  ...
  db.rooms.forEach((room, i) => {
    const usedBy = db.seatingPlans.filter((p) => p.roomId === room.id).map((p) => className(p.classId)).filter(Boolean);
    const row = document.createElement("div");
    row.className = "list-header";
    row.style.borderBottom = i === db.rooms.length - 1 ? "none" : "1px solid var(--rule)";
    row.innerHTML = `
      <div>
        <div class="row-title">${room.name}</div>
        <div class="row-sub">${room.desks.length} table(s)${usedBy.length ? " · utilisée par " + usedBy.join(", ") : ""}</div>
      </div>
      <div style="display:flex;gap:6px;">
        <button class="btn btn-ghost btn-sm" data-edit>Modifier le plan</button>
        <button class="btn btn-danger btn-sm" data-del>Supprimer</button>
      </div>`;
    row.querySelector("[data-edit]").addEventListener("click", () => { roomsEditingId = room.id; render(); });
    row.querySelector("[data-del]").addEventListener("click", async () => {
      if (!confirm(`Supprimer la salle « ${room.name} » ? Les plans de classe qui l'utilisent perdront leur salle assignée.`)) return;
      db.rooms = db.rooms.filter((r) => r.id !== room.id);
      db.seatingPlans.forEach((p) => { if (p.roomId === room.id) { p.roomId = null; p.seats = []; } });
      await save(db);
      render();
    });
...
function renderRoomEditor(main, room) {
  ensureRoomFurniture(room);
  pageHead(main, "Salles", room.name,
    `<button class="btn btn-ghost" id="back-to-rooms">← Retour aux salles</button>
     ${toggleZoomButtonHtml()}
     <button class="btn btn-primary" id="add-desk-btn">+ Ajouter une table</button>`);
  main.querySelector("#back-to-rooms").addEventListener("click", () => { roomsEditingId = null; render(); });
  bindZoomToggle(main);
  ...
  const p = document.createElement("p");
  p.style.cssText = "color:var(--ink-soft);font-size:12.5px;margin-bottom:12px;";
  p.textContent = "Glisse chaque table pour reproduire la disposition réelle de la salle, vue du dessus. Le tableau et le bureau du prof sont aussi déplaçables. Les places s'attribuent ensuite depuis « Plan de classe ».";
  main.appendChild(p);
  ...
  canvas.style.width = room.canvasW + "px";
  canvas.style.height = room.canvasH + "px";

  const boardEl = document.createElement("div");
  boardEl.className = "room-furniture board";
  boardEl.style.left = room.board.x + "px";
  boardEl.style.top = room.board.y + "px";
  boardEl.textContent = "Tableau";
  makeDeskDraggable(boardEl, room.board, canvas, async () => { await save(db); }, BOARD_W, BOARD_H, room.canvasW, room.canvasH);
  canvas.appendChild(boardEl);

  const teacherEl = document.createElement("div");
  teacherEl.className = "room-furniture teacher-desk";
  teacherEl.style.left = room.teacherDesk.x + "px";
  teacherEl.style.top = room.teacherDesk.y + "px";
  teacherEl.textContent = "Bureau";
  makeDeskDraggable(teacherEl, room.teacherDesk, canvas, async () => { await save(db); }, TEACHER_DESK_W, TEACHER_DESK_H, room.canvasW, room.canvasH);
  ...
  const sizer = wrapCanvasForZoom(canvasWrap, canvas, room.canvasW, room.canvasH);

  // Poignée pour étirer la surface de la salle (coin bas-droit), afin de faire de la place pour d'autres tables
  const resizeHandle = document.createElement("div");
  resizeHandle.className = "room-resize-handle";
  resizeHandle.title = "Étirer pour agrandir la salle";
  resizeHandle.textContent = "⤡";
  sizer.appendChild(resizeHandle);
  makeCanvasResizable(resizeHandle, room, canvas, sizer, async () => { await save(db); });

  main.appendChild(canvasWrap);
}

// Glisser la poignée en bas à droite pour étirer la surface de la salle (ajoute de l'espace pour d'autres tables).
function makeCanvasResizable(handle, room, canvas, sizer, onDrop) {
  let dragging = false, startX = 0, startY = 0, startW = 0, startH = 0;
  handle.addEventListener("pointerdown", (e) => {
    dragging = true;
    handle.setPointerCapture(e.pointerId);
    startX = e.clientX; startY = e.clientY;
    startW = room.canvasW; startH = room.canvasH;
  });
  handle.addEventListener("pointermove", (e) => {
    if (!dragging) return;
    const dx = (e.clientX - startX) / planZoom;
    const dy = (e.clientY - startY) / planZoom;
    room.canvasW = Math.max(CANVAS_MIN_W, Math.min(CANVAS_MAX_W, startW + dx));
    room.canvasH = Math.max(CANVAS_MIN_H, Math.min(CANVAS_MAX_H, startH + dy));
    canvas.style.width = room.canvasW + "px";
    canvas.style.height = room.canvasH + "px";
    sizer.style.width = (room.canvasW * planZoom) + "px";
    sizer.style.height = (room.canvasH * planZoom) + "px";
  });
...
// Glisser-déposer au pointeur (souris + tactile unifiés) — reste dans les limites du canvas.
function makeDeskDraggable(el, desk, canvas, onDrop, w = DESK_W, h = DESK_H, canvasW, canvasH) {
  ...
  el.addEventListener("pointermove", (e) => {
    ...
    let x = (e.clientX - canvasRect.left - offsetX) / planZoom;
    let y = (e.clientY - canvasRect.top - offsetY) / planZoom;
...
/* ================= PLAN DE CLASSE (assignation des élèves) ================= */
let seatingSelectedClassId = null;
let seatingHeldStudentId = null;

function getOrCreateSeatingPlan(classId) {
  let plan = db.seatingPlans.find((p) => p.classId === classId);
  if (!plan) {
    plan = { id: uid(), classId, roomId: null, seats: [] };
    db.seatingPlans.push(plan);
  }
  return plan;
}

function desksAreClose(a, b) {
  const dx = a.x - b.x, dy = a.y - b.y;
  return Math.sqrt(dx * dx + dy * dy) < ADJACENT_DIST;
}
...
function renderSeating(main) {
  if (db.classes.length === 0) {
    pageHead(main, "Organisation", "Plan de classe");
    main.appendChild(emptyState("Aucune classe", "Crée d'abord une classe."));
    return;
  }
  if (!seatingSelectedClassId || !db.classes.some((c) => c.id === seatingSelectedClassId)) {
    seatingSelectedClassId = db.classes[0].id;
  }
  const classId = seatingSelectedClassId;
  const plan = getOrCreateSeatingPlan(classId);
  const room = plan.roomId ? db.rooms.find((r) => r.id === plan.roomId) : null;
  const students = db.students.filter((s) => s.classId === classId);
  const placedIds = new Set(plan.seats.map((s) => s.studentId));
  const unplaced = students.filter((s) => !placedIds.has(s.id));
  const constraints = db.seatingConstraints.filter((c) => c.classId === classId);

  const classOptions = db.classes.map((c) => `<option value="${c.id}" ${c.id === classId ? "selected" : ""}>${c.name}</option>`).join("");
  const roomOptions = `<option value="">— Choisir une salle —</option>` + db.rooms.map((r) => `<option value="${r.id}" ${r.id === plan.roomId ? "selected" : ""}>${r.name}</option>`).join("");

  pageHead(
    main, "Organisation", "Plan de classe",
    `<select id="seating-class-select" style="padding:8px 10px;border:1px solid var(--rule);border-radius:7px;background:var(--surface);font-size:13.5px;">${classOptions}</select>
     <select id="seating-room-select" style="padding:8px 10px;border:1px solid var(--rule);border-radius:7px;background:var(--surface);font-size:13.5px;">${roomOptions}</select>
     <button class="btn btn-primary btn-sm" id="seating-shuffle">🎲 Générer aléatoirement</button>
     <button class="btn btn-ghost btn-sm" id="seating-clear">Vider le plan</button>
     <button class="btn btn-ghost btn-sm" id="seating-print">🖨️ Imprimer ce plan</button>
     ${toggleZoomButtonHtml()}`
  );
  main.querySelector("#seating-class-select").addEventListener("change", (e) => {
    seatingSelectedClassId = e.target.value;
    seatingHeldStudentId = null;
    render();
  });
  main.querySelector("#seating-room-select").addEventListener("change", async (e) => {
    plan.roomId = e.target.value || null;
    plan.seats = [];
    await save(db);
    render();
  });
  main.querySelector("#seating-shuffle").addEventListener("click", async () => {
    if (!room) { alert("Choisis d'abord une salle."); return; }
    if (students.length === 0) { alert("Aucun élève dans cette classe."); return; }
    if (students.length > room.desks.length) {
      alert(`Pas assez de tables (${room.desks.length}) pour ${students.length} élève(s). Ajoute des tables dans « Salles ».`);
      return;
    }
    let best = null, bestViolations = Infinity;
    for (let attempt = 0; attempt < 300; attempt++) {
      const desks = shuffleArray(room.desks);
      const order = shuffleArray(students);
      const assignment = order.map((s, i) => ({ studentId: s.id, deskId: desks[i].id }));
      let violations = 0;
      constraints.forEach((ct) => {
        const aAssign = assignment.find((a) => a.studentId === ct.studentA);
        const bAssign = assignment.find((a) => a.studentId === ct.studentB);
        if (aAssign && bAssign) {
          const deskA = room.desks.find((d) => d.id === aAssign.deskId);
          const deskB = room.desks.find((d) => d.id === bAssign.deskId);
          if (deskA && deskB && desksAreClose(deskA, deskB)) violations++;
        }
      });
    ...
    await save(db);
    render();
    if (bestViolations === 0) showToast("Plan généré — toutes les contraintes sont respectées ✓");
    else showToast(`Plan généré, mais ${bestViolations} contrainte(s) n'a/n'ont pas pu être respectée(s).`);
  });
  main.querySelector("#seating-clear").addEventListener("click", async () => {
    if (!confirm("Vider entièrement ce plan de classe ?")) return;
    plan.seats = [];
    await save(db);
    render();
  });
  ...
  if (db.rooms.length === 0) {
    main.appendChild(emptyState("Aucune salle définie", "Va dans « Salles » pour créer et dessiner ta première salle avant de pouvoir y placer des élèves."));
    return;
  }
  if (!room) {
    main.appendChild(emptyState("Choisis une salle", "Utilise le menu déroulant ci-dessus pour attribuer une salle à cette classe."));
    return;
  }
  ...
  layout.className = "seating-layout";

  const roomCard = document.createElement("div");
  roomCard.className = "card";
  roomCard.innerHTML = `<h3>${room.name} — ${className(classId)}</h3><p style="font-size:12px;color:var(--ink-soft);margin-bottom:10px;">Clique sur un élève (ci-dessous ou dans la liste à droite) pour le "prendre", puis clique sur une table pour l'y déposer.</p>`;
  const canvasWrap = document.createElement("div");
  canvasWrap.className = "room-canvas-wrap";
  const canvas = document.createElement("div");
  canvas.className = "room-canvas";
  ...
  room.desks.forEach((desk) => {
    ...
    const deskEl = document.createElement("div");
    deskEl.className = "desk" + (occupant ? " occupied" : " empty");
    ...
    deskEl.addEventListener("click", async () => {
      if (seatingHeldStudentId) {
        if (occupant && occupant.id === seatingHeldStudentId) {
          ...
          plan.seats = plan.seats.filter((s) => s.studentId !== seatingHeldStudentId && s.deskId !== desk.id);
          plan.seats.push({ deskId: desk.id, studentId: seatingHeldStudentId });
          seatingHeldStudentId = null;
          await save(db);
        }
        render();
      } else if (occupant) {
        seatingHeldStudentId = occupant.id;
        render();
      }
    });
    canvas.appendChild(deskEl);
  ...
  // Feuille dédiée à l'impression : clonée AVANT le zoom, toujours à taille réelle sur papier
  const printSheet = document.createElement("div");
  printSheet.className = "print-only print-sheet";
  printSheet.innerHTML = `<h2 style="margin-bottom:14px;">${room.name} — ${className(classId)}</h2>`;
  const printCanvas = canvas.cloneNode(true);
  printCanvas.style.transform = "none";
  printCanvas.querySelectorAll(".desk, .room-furniture").forEach((el) => el.style.cursor = "default");
  printSheet.appendChild(printCanvas);
  main.appendChild(printSheet);
  main.querySelector("#seating-print").addEventListener("click", () => printSingleElement(printSheet));

  wrapCanvasForZoom(canvasWrap, canvas, room.canvasW, room.canvasH);
  roomCard.appendChild(canvasWrap);
  layout.appendChild(roomCard);

  const sideCol = document.createElement("div");
  sideCol.style.cssText = "display:flex;flex-direction:column;gap:16px;";

  const rosterCard = document.createElement("div");
  rosterCard.className = "card";
  rosterCard.innerHTML = `<h3>Élèves non placés (${unplaced.length})</h3>`;
  const rosterList = document.createElement("div");
  rosterList.className = "row-list";
  if (unplaced.length === 0) {
    rosterList.innerHTML = `<div class="empty-day">Tout le monde est placé 🎉</div>`;
  } else {
    unplaced.forEach((s) => {
      const item = document.createElement("div");
      item.className = "roster-chip" + (s.id === seatingHeldStudentId ? " held" : "");
      item.textContent = s.name;
      item.addEventListener("click", () => {
        seatingHeldStudentId = s.id === seatingHeldStudentId ? null : s.id;
        render();
      });
  ...
  sideCol.appendChild(rosterCard);

  const constraintsCard = document.createElement("div");
  constraintsCard.className = "card";
  constraintsCard.innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
      <h3 style="margin:0;">Ne pas placer ensemble</h3>
      <button class="btn btn-ghost btn-sm" id="add-constraint-btn">+ Ajouter</button>
    </div>`;
  const constraintsBody = document.createElement("div");
  constraintsBody.className = "row-list";
  if (constraints.length === 0) {
    constraintsBody.innerHTML = `<div class="empty-day">Aucune contrainte définie.</div>`;
  } else {
    constraints.forEach((ct) => {
      const nameA = db.students.find((s) => s.id === ct.studentA)?.name || "?";
      const nameB = db.students.find((s) => s.id === ct.studentB)?.name || "?";
      const row = document.createElement("div");
      row.className = "task-item";
      row.innerHTML = `<div class="task-title" style="font-size:13px;">${nameA} ↔ ${nameB}</div><button class="btn btn-danger btn-sm" data-del>✕</button>`;
      row.querySelector("[data-del]").addEventListener("click", async () => {
        db.seatingConstraints = db.seatingConstraints.filter((x) => x.id !== ct.id);
        await save(db);
        render();
      });
      constraintsBody.appendChild(row);
    });
  }
  constraintsCard.appendChild(constraintsBody);
  sideCol.appendChild(constraintsCard);

  layout.appendChild(sideCol);
  main.appendChild(layout);

  constraintsCard.querySelector("#add-constraint-btn").addEventListener("click", () => openConstraintModal(classId, students));
}

function openConstraintModal(classId, students) {
  if (students.length < 2) {
    openModal(`<h2>Pas assez d'élèves</h2><p style="color:var(--ink-soft);font-size:13.5px;">Il faut au moins 2 élèves dans la classe pour créer une contrainte.</p><div class="modal-actions"><button class="btn btn-primary" id="f-cancel">Fermer</button></div>`);
    document.getElementById("f-cancel").onclick = closeModal;
    return;
  }
  const options = students.map((s) => `<option value="${s.id}">${s.name}</option>`).join("");
  openModal(`
    <h2>Ne pas placer ensemble</h2>
    <div class="field"><label>Élève 1</label><select id="f-student-a">${options}</select></div>
    <div class="field"><label>Élève 2</label><select id="f-student-b">${options}</select></div>
    <div class="modal-actions">
      <button class="btn btn-ghost" id="f-cancel">Annuler</button>
  ...
  document.getElementById("f-cancel").onclick = closeModal;
  document.getElementById("f-save").onclick = async () => {
    const a = document.getElementById("f-student-a").value;
    const b = document.getElementById("f-student-b").value;
    if (a === b) { alert("Choisis deux élèves différents."); return; }
    const exists = db.seatingConstraints.some((c) =>
      c.classId === classId && ((c.studentA === a && c.studentB === b) || (c.studentA === b && c.studentB === a))
    );
    if (exists) { alert("Cette contrainte existe déjà."); return; }
    db.seatingConstraints.push({ id: uid(), classId, studentA: a, studentB: b });
    await save(db);
    closeModal();
    render();
  };
...
function renderClasses(main) {
  ...
  db.classes.forEach((cls) => {
    ...
    card.querySelector("[data-delete-class]").addEventListener("click", async () => {
...
let evalsGroupByClass = localStorage.getItem("profplanner_evals_grouped") === "1";
let evalsCollapsedGroups = new Set(JSON.parse(localStorage.getItem("profplanner_evals_collapsed") || "[]"));
let evalsCollapsedCards = new Set(JSON.parse(localStorage.getItem("profplanner_evals_card_collapsed") || "[]"));
function toggleEvalCardCollapse(evalId) {
  if (evalsCollapsedCards.has(evalId)) evalsCollapsedCards.delete(evalId);
  else evalsCollapsedCards.add(evalId);
  localStorage.setItem("profplanner_evals_card_collapsed", JSON.stringify([...evalsCollapsedCards]));
  render();
}
function toggleEvalGroupCollapse(key) {
  if (evalsCollapsedGroups.has(key)) evalsCollapsedGroups.delete(key);
  else evalsCollapsedGroups.add(key);
  localStorage.setItem("profplanner_evals_collapsed", JSON.stringify([...evalsCollapsedGroups]));
  render();
}

function buildEvalGroupHeader(key, label, count, color) {
  const collapsed = evalsCollapsedGroups.has(key);
  const header = document.createElement("div");
  header.className = "eval-group-header";
  header.style.color = color;
  header.innerHTML = `<span class="eval-group-arrow">${collapsed ? "▸" : "▾"}</span> ${label} <span class="eval-group-count">(${count})</span>`;
  header.addEventListener("click", () => toggleEvalGroupCollapse(key));
  return header;
}
...
function renderEvals(main) {
  pageHead(main, "Notes", "Suivi des évaluations",
    `<div class="view-toggle">
       <button class="toggle-btn ${!evalsGroupByClass ? "active" : ""}" id="evals-mode-chrono">Chronologique</button>
       <button class="toggle-btn ${evalsGroupByClass ? "active" : ""}" id="evals-mode-grouped">Par classe</button>
     </div>
     ${evalsGroupByClass ? `
       <button class="btn btn-ghost btn-sm" id="evals-collapse-all">Replier les classes</button>
       <button class="btn btn-ghost btn-sm" id="evals-expand-all">Déplier les classes</button>` : ""}
     <button class="btn btn-ghost btn-sm" id="evals-collapse-cards">Réduire les notes</button>
     <button class="btn btn-ghost btn-sm" id="evals-expand-cards">Détailler les notes</button>
  ...
  main.querySelector("#evals-mode-chrono").addEventListener("click", () => {
    evalsGroupByClass = false;
    localStorage.setItem("profplanner_evals_grouped", "0");
    render();
  });
  main.querySelector("#evals-mode-grouped").addEventListener("click", () => {
    evalsGroupByClass = true;
    localStorage.setItem("profplanner_evals_grouped", "1");
    render();
  });
  main.querySelector("#evals-collapse-cards").addEventListener("click", () => {
    evalsCollapsedCards = new Set(db.evaluations.map((e) => e.id));
    localStorage.setItem("profplanner_evals_card_collapsed", JSON.stringify([...evalsCollapsedCards]));
    render();
  });
  main.querySelector("#evals-expand-cards").addEventListener("click", () => {
    evalsCollapsedCards = new Set();
    localStorage.setItem("profplanner_evals_card_collapsed", JSON.stringify([]));
    render();
  });
  if (evalsGroupByClass) {
    main.querySelector("#evals-collapse-all").addEventListener("click", () => {
      evalsCollapsedGroups = new Set([...db.classes.map((c) => c.id), "__orphan__"]);
      localStorage.setItem("profplanner_evals_collapsed", JSON.stringify([...evalsCollapsedGroups]));
      render();
    });
    main.querySelector("#evals-expand-all").addEventListener("click", () => {
      evalsCollapsedGroups = new Set();
      localStorage.setItem("profplanner_evals_collapsed", JSON.stringify([]));
      render();
    });
  }
  ...
  if (evalsGroupByClass) {
    db.classes.forEach((cls) => {
      const evalsForClass = db.evaluations.filter((e) => e.classId === cls.id).sort((a, b) => b.date.localeCompare(a.date));
      if (evalsForClass.length === 0) return;
      main.appendChild(buildEvalGroupHeader(cls.id, cls.name, evalsForClass.length, "var(--board)"));
      if (!evalsCollapsedGroups.has(cls.id)) {
        evalsForClass.forEach((ev) => main.appendChild(buildEvalCard(ev)));
      }
    });
    const orphan = db.evaluations.filter((e) => !db.classes.some((c) => c.id === e.classId));
    if (orphan.length > 0) {
      main.appendChild(buildEvalGroupHeader("__orphan__", "Sans classe", orphan.length, "var(--ink-soft)"));
      if (!evalsCollapsedGroups.has("__orphan__")) {
        orphan.forEach((ev) => main.appendChild(buildEvalCard(ev)));
      }
    }
  } else {
    [...db.evaluations].sort((a, b) => b.date.localeCompare(a.date)).forEach((ev) => main.appendChild(buildEvalCard(ev)));
  }
}
...
function buildEvalCard(ev) {
  ...
  card.className = "list-card";
  card.style.marginBottom = "14px";
  const isCollapsed = evalsCollapsedCards.has(ev.id);
  card.innerHTML = `
    <div class="list-header eval-card-header" style="cursor:pointer;">
      <div>
        <h3><span class="eval-group-arrow">${isCollapsed ? "▸" : "▾"}</span> ${ev.title} ${isColorMode ? '<span class="pill pill-info">Couleurs</span>' : ""}</h3>
