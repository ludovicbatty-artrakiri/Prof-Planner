...
const DB_KEY = "prof'planner_db_v1";
...
function renderYearOverview(main) {
  ...
  main.appendChild(buildYearOverviewGrid(calendarYear));
}

function buildYearOverviewGrid(year) {
...
function renderCalendar(main) {
  ...
  legend.innerHTML = Object.values(EVENT_TYPES).map((t) => `
    ...
    <div style="display:flex;align-items:center;gap:6px;font-size:12px;color:var(--ink-soft);">
      <span class="mono" style="text-decoration:line-through;">12</span> jour déjà passé
    </div>`;
  main.appendChild(legend);
...
function renderPrep(main) {
  ...
  if (db.preparations.length === 0) {
    main.appendChild(emptyState("Aucune fiche de préparation", "Clique sur « + Ajouter une préparation » pour créer ta première fiche — pas besoin d'avoir déjà un cours dans le planning."));
    return;
  }
...
window.addEventListener("appinstalled", () => {
  ...
  showToast("Prof'Planner est installé 🎉");
});
  ...
  if (isIOS) {
    ...
    steps = `
      ...
      <ol style="padding-left:20px;line-height:1.9;">
        ...
        <li>Choisis <strong>« Installer Prof'Planner »</strong> ou <strong>« Ajouter à l'écran d'accueil »</strong>.</li>
      </ol>`;
  }
  openModal(`
    <h2>Installer Prof'Planner</h2>
    ${steps}
