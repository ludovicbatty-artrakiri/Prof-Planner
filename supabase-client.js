...
async function cloudSignUp(email, password) {
  ...
  // Crée la ligne de profil correspondante (si la session est déjà active)
  if (data.user) {
...
async function cloudFetchAllData() {
  const [
    ...
    sb.from("seating_plans").select("*"),
    sb.from("seating_seats").select("*"),
    sb.from("seating_constraints").select("*"),
  ...
  const roomDesksRaw = roomDesksRes.data || [];
  const rooms = (roomsRes.data || []).map((r) => ({
    id: r.id,
    name: r.name,
    canvasW: r.canvas_w ? Number(r.canvas_w) : 720,
    canvasH: r.canvas_h ? Number(r.canvas_h) : 420,
    board: (r.board_x !== null && r.board_y !== null) ? { x: Number(r.board_x), y: Number(r.board_y) } : null,
  ...
  const seatingPlans = (seatingPlansRes.data || []).map((p) => ({
    id: p.id,
    classId: p.class_id,
    roomId: p.room_id,
    seats: seatingSeatsRaw
      .filter((s) => s.seating_plan_id === p.id)
      .map((s) => ({ deskId: s.desk_id, studentId: s.student_id })),
  }));

  const seatingConstraints = (seatingConstraintsRes.data || []).map((c) => ({
    id: c.id, classId: c.class_id, studentA: c.student_a, studentB: c.student_b,
  }));

  return { timeSlots, classes, students, courses, preparations, tasks, evaluations, events, links, documents, rooms, seatingPlans, seatingConstraints };
}
...
async function cloudSyncAll(db, userId) {
  ...
  // salles (owner-based)
  await syncSimpleTable("rooms", db.rooms, (r) => ({
    id: r.id, owner_id: userId, name: r.name,
    canvas_w: r.canvasW || 720, canvas_h: r.canvasH || 420,
    board_x: r.board ? r.board.x : null, board_y: r.board ? r.board.y : null,
    teacher_x: r.teacherDesk ? r.teacherDesk.x : null, teacher_y: r.teacherDesk ? r.teacherDesk.y : null,
  }));

  // tables de chaque salle (structure imbriquée -> on retire tout puis on réinsère, par salle)
  try {
    ...
    console.error("[sync] room_desks", e);
  }

  // plan de classe : plans + contraintes (upsert/delete standard)
  await syncSimpleTable("seating_plans", db.seatingPlans, (p) => ({
    id: p.id, owner_id: userId, class_id: p.classId, room_id: p.roomId || null,
  }));
  await syncSimpleTable("seating_constraints", db.seatingConstraints, (c) => ({
    id: c.id, owner_id: userId, class_id: c.classId, student_a: c.studentA, student_b: c.studentB,
  }));

  // plan de classe : places assises (structure imbriquée -> on retire tout puis on réinsère, par plan)
  try {
    for (const plan of db.seatingPlans) {
      await sb.from("seating_seats").delete().eq("seating_plan_id", plan.id);
      if (plan.seats && plan.seats.length) {
        const rows = plan.seats.map((s) => ({
          seating_plan_id: plan.id, desk_id: s.deskId, student_id: s.studentId,
        }));
        const { error } = await sb.from("seating_seats").insert(rows);
        if (error) console.error("[sync] seating_seats", error.message);
      }
    }
  } catch (e) {
    console.error("[sync] seating_seats", e);
  }
}
